...
class Start(Task2State):
    ...
    def execute(self, userdata):
        ...
        while not (self.robot.getFlightState() == self.robot.HOVER_STATE):
            pass
        self.robot.goPosWaitConvergence('global', None, self.object_lookdown_height, None, timeout=10, pos_conv_thresh = 0.4, yaw_conv_thresh = 0.2, vel_conv_thresh = 0.2)
...
class ApproachPickPosition(Task2State):
    ...
    def execute(self, userdata):
        ...
        #enable alt sensor
        if self.disable_alt_sensor and (not self.simulation):
            try:
...
class LookDown(Task2State):
    ...
    def execute(self, userdata):
        ...
        if self.no_lookdown_mode:
            ...
            self.robot.goPosWaitConvergence('global', self.global_lookdown_pos_gps, self.object_approach_height, None, gps_mode=True, pos_conv_thresh = 0.2, yaw_conv_thresh = 0.1, vel_conv_thresh = 0.1, timeout = 10)
            return 'succeeded'
        ...
        if object_found:
            ...
            if abs(object_yaw - (-np.pi / 2)) > self.object_yaw_thresh:
                rospy.logerr(self.__class__.__name__ + ": object invalid yaw")
                object_found = False
        ...
        if object_found:
            ...
            self.robot.goPosWaitConvergence('global', [object_global_pos[0], object_global_pos[1]], None, uav_target_yaw, pos_conv_thresh = 0.2, yaw_conv_thresh = 0.2, vel_conv_thresh = 0.1)
            ...
            if userdata.search_failed:
                ...
                #go to next state
                self.robot.goPosWaitConvergence('global', None, self.object_approach_height, None, pos_conv_thresh = 0.2, yaw_conv_thresh = 0.1, vel_conv_thresh = 0.1)
...
class AdjustGraspPosition(Task2State):
    ...
    def execute(self, userdata):
        ...
        if object_found:
            ...
            self.robot.goPosWaitConvergence('global', [object_global_pos[0], object_global_pos[1]], None, uav_target_yaw, pos_conv_thresh = 0.15, yaw_conv_thresh = 0.05, vel_conv_thresh = 0.1)
            #reset search state
            userdata.search_count = 0
            userdata.search_failed = False
            ...
            self.robot.preshape()
            self.robot.goPosWaitConvergence('global', [uav_target_pos[0], uav_target_pos[1]], None, uav_target_yaw, pos_conv_thresh = 0.01, yaw_conv_thresh = 0.05, vel_conv_thresh = 0.1)
            return 'succeeded'
...
class Grasp(Task2State):
    ...
    def execute(self, userdata):
        ...
        if not self.do_object_recognition:
            ...
            self.robot.preshape()
            self.robot.goPosWaitConvergence('global', [uav_target_pos[0], uav_target_pos[1]], None, uav_target_yaw, pos_conv_thresh = 0.1, yaw_conv_thresh = 0.1, vel_conv_thresh = 0.1)
        ...
        if self.grasp_land_mode and (not self.grasp_force_landing_mode):
            ...
            if self.grasp_force_landing_mode:
                self.robot.goPosWaitConvergence('global', None, self.force_landing_height, None, pos_conv_thresh = 0.15, yaw_conv_thresh = 0.05, vel_conv_thresh = 0.1, timeout=15)
                ...
                #descend
                self.robot.goPosWaitConvergence('global', None, self.object_grasping_height, None, pos_conv_thresh = 0.2, yaw_conv_thresh = 0.1, vel_conv_thresh = 0.1)
...
class ApproachPlacePosition(Task2State):
    ...
    def execute(self, userdata):
        ...
        self.robot.goPosWaitConvergence('global', None, self.global_place_channel_z + self.place_z_offset, None, pos_conv_thresh = 0.4, yaw_conv_thresh = 0.2, vel_conv_thresh = 0.2)
...
class Ungrasp(Task2State):
    ...
    def execute(self, userdata):
        ...
        #descend
        self.robot.goPosWaitConvergence('global', None, self.global_place_channel_z + self.place_z_margin, None, pos_conv_thresh = 0.15, yaw_conv_thresh = 0.1, vel_conv_thresh = 0.1, timeout = 10)

        self.robot.ungrasp()
        self.remove_object_model_func(self.robot)
        userdata.object_count += 1
        self.robot.goPosWaitConvergence('global', None, self.global_place_channel_z + self.place_z_offset, None, pos_conv_thresh = 0.3, yaw_conv_thresh = 0.2, vel_conv_thresh = 0.3)
